...
class HealthCheckTask(CeleryTask):
    ...
    def on_failure(self, exc, task_id, args, kwargs, einfo):
        """Error handling"""
        logger.error(
            f"Health check task {task_id} failed: {exc}. Traceback: {traceback.format_exc()}"
        )
...
def run_health_check_task(self, model_id: str) -> Dict:
    """
    Check model availability via Celery
...
async def _health_check_async(model_id: str) -> Dict:
    ...
    try:
        ...
        logger.error(
            f"Health check failed for model {model_id}: {e}. Traceback: {traceback.format_exc()}"
        )
        return {
            "success": False,
            "error": str(e),
...
async def _test_inference_async(model_id: str, test_prompt: str) -> Dict:
    ...
    try:
        ...
        logger.info(
            f"Test inference completed in {duration:.2f}s: Response: {response}"
        )
        ...
        logger.error(
            f"Test inference failed for model {model.name}: {e}. Traceback: {traceback.format_exc()}"
        )
